...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^dos_") ; then
        CM_BUILD=$(echo -n $1 | sed -e 's/^dos_//g')
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        cd $T > /dev/null
        vendor/dos/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/dos/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/dos/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
